const db = require('../config/db');
const logAction = require('../utils/logger');

const assignEquipment = async (req, res) => {
    const { equipment_id, personnel_name, base_id, quantity, assigned_date } = req.body;
    const assigned_by = req.user.id;

    if (!equipment_id || isNaN(equipment_id) || parseInt(equipment_id) <= 0) {
        return res.status(400).json({ message: "A valid positive Equipment ID is required." });
    }
    if (!personnel_name || !personnel_name.trim()) {
        return res.status(400).json({ message: "Personnel name is required." });
    }
    if (!base_id || isNaN(base_id) || parseInt(base_id) <= 0) {
        return res.status(400).json({ message: "A valid positive Base ID is required." });
    }
    if (quantity === undefined || isNaN(quantity) || parseInt(quantity) <= 0) {
        return res.status(400).json({ message: "Quantity assigned must be greater than 0." });
    }

    try {
        const eqResult = await db.query('SELECT * FROM equipment WHERE id = $1', [equipment_id]);
        if (eqResult.rows.length === 0) {
            return res.status(404).json({ message: "Equipment not found." });
        }

        const equipment = eqResult.rows[0];

        if (equipment.base_id !== parseInt(base_id)) {
            return res.status(400).json({ message: "Equipment does not belong to this base." });
        }

        if (equipment.quantity < parseInt(quantity)) {
            return res.status(400).json({ message: "Insufficient stock available to assign." });
        }

        await db.query('BEGIN');

        await db.query(
            'UPDATE equipment SET quantity = quantity - $1 WHERE id = $2',
            [quantity, equipment_id]
        );

        const assignmentResult = await db.query(
            `INSERT INTO assignments (equipment_id, personnel_name, base_id, quantity, assigned_date, assigned_by) 
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [equipment_id, personnel_name, base_id, quantity, assigned_date || new Date(), assigned_by]
        );

        await db.query('COMMIT');

        await logAction(
            assigned_by,
            `Assigned ${quantity} units of ${equipment.name} to ${personnel_name} at base ID ${base_id}`
        );

        res.status(201).json({
            message: "Equipment assigned successfully!",
            assignment: assignmentResult.rows[0]
        });

    } catch (error) {
        await db.query('ROLLBACK');
        console.error(error);
        res.status(500).json({ message: "Server error during assignment." });
    }
};

const getAssignmentHistory = async (req, res) => {
    try {
        let query = `
      SELECT a.*, e.name AS equipment_name, e.type AS equipment_type 
      FROM assignments a
      JOIN equipment e ON a.equipment_id = e.id
      WHERE 1=1
    `;
        const values = [];
        let idx = 0;

        if (req.user.role === 'BaseCommander') {
            const userResult = await db.query('SELECT base_id FROM users WHERE id = $1', [req.user.id]);
            const userBaseId = userResult.rows[0]?.base_id;

            if (!userBaseId) {
                return res.status(403).json({ message: "Access denied. Commander is not assigned to any base." });
            }

            idx++;
            values.push(userBaseId);
            query += ` AND a.base_id = $${idx}`;
        } else if (req.query.base_id) {
            idx++;
            values.push(req.query.base_id);
            query += ` AND a.base_id = $${idx}`;
        }

        query += ` ORDER BY a.assigned_date DESC`;

        const result = await db.query(query, values);
        res.status(200).json(result.rows);

    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Server error while fetching assignments." });
    }
};

module.exports = {
    assignEquipment,
    getAssignmentHistory
};
